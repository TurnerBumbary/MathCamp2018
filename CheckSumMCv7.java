...
public class CheckSumMCv7 {
	...
	public static void main(String[] args) {
		...
		CheckSumMCv7 queueTest = new CheckSumMCv7();
		queueTest.test();
	}
	...
	class Producer implements Runnable {
		...
		public void run() {
			try {
				...
				System.out
						.println("Monitor state before join: " + m.getState());
				m.join();
				System.out.println("Monitor state after join: " + m.getState());
				if (m.getState().equals(Thread.State.NEW)) {
					System.out.println("Starting Monitor");
					m.start();
					System.out.println("Start has concluded");
				} else {
					System.out.println("Running Monitor");
					m.run();
					System.out.println("Run has concluded");
				}
				System.out.println("Monitor state after start/run: "
						+ m.getState());
				m.join();
